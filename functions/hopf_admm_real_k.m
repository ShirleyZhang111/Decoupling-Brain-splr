function [x,Z,u,v,res,flag0] = hopf_admm_real_k(A,B,C,mu,la,maxit,tol,k)
%HOPF_ADMM solves
%   min sum_j 0.5*norm(A(:,:,j)*x + (Y+uv')*B(:,j) - C(:,j))^2 + mu*norm(Z,ell_1) + la*norm(v,2), 
%        s.t. Z = Y
%Input:
...
for iter = 1:maxit
    ...
    % update x
    x = dG\(x.*tau + A'*reshape(C-(Y+u0*v0')*B,[],1));

    % update Y
    W = reshape(A*x,[n,t]);
    Y = (Y.*tau + Z.*rho - lambda + ...
        (real(W-C)+u0*v0'*real(B))*real(B)'+...
        + (imag(W-C)+u0*v0'*imag(B))*imag(B)')/dF;
    ...
    s0 = -((real(W-C)+Y*real(B))*(real(B)'*v0)+(imag(W-C)+Y*imag(B))*(imag(B)'*v0) - tau*u0);
    % K = W+YB-C
    [u_l,~,v_l] = svd(s0,'econ');
    u = u_l*v_l';
    
    s0 = (-real(B)*(real(W-C)+Y*real(B))'- imag(B)*(imag(W-C)+Y*imag(B))')*u0 + tau*v0;
    v = dF0\s0;
    ...
    res(4,iter) = norm(u*v' - u0*v0', 'fro') / sqrtnt;
    ...
    % adapt penalty
    % if res(1,iter) < mean(res(2:4,iter))*0.2
    %     rho = rho*1.1;
    %     flag = 1;
    % elseif res(1,iter) > mean(res(2:4,iter))*5
    %     rho = rho/1.1;
    %     flag = 1;
    % else
    %     flag = 0;
    % end
    if res(1,iter) > 10 * res(2,iter) 
        rho = rho * 1.618;  
        flag = 1;
    elseif res(2,iter) > 10 * res(1,iter)
        rho = rho / 1.618;
        flag = 1;
    else
        flag = 0;
